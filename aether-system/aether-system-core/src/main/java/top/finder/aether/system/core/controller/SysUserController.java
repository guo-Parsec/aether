...
@Api(tags = "系统用户管理")
@RestController
@RequestMapping(value = SystemApiConstantPool.USER_WEB_API_PREFIX)
public class SysUserController {
    ...
    @AppGroup(blocking = {
            @AppBlocking(appNames = {AppConstantPool.APP_NAME_SYSTEM}, blockerId = "userService.systemInnerUserCreate"),
    ...
    public Apis<Void> enableUser(@RequestParam(value = "account") String account) {
        service.changeUserEnableStatus(account, SystemConstantPool.ENABLE_STATUS_ENABLE);
        return Apis.success();
    }
    ...
    public Apis<Void> disableUser(@RequestParam(value = "account") String account) {
        service.changeUserEnableStatus(account, SystemConstantPool.ENABLE_STATUS_DISABLE);
        return Apis.success();
    }
